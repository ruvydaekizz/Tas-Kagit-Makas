...
def game_loop():  # Oyun döngüsünü başlatan fonksiyon
    ...
    while True:  # Sonsuz döngü, oyuncu oyunu bitirene kadar devam eder
        ...
        while player_tour_win < 2 and computer_tour_win < 2:  # Oyuncu veya bilgisayar iki tur kazanana kadar döngü devam ede
            ...
            player_choice = input(
                " 'rock', 'paper', 'scissors' or if you want to leave 'exit': ").lower()  # oyuncuya dışarıdan değer girdiriyoruz
            ...
            while player_choice not in choices:  # Oyuncunun girişi geçerli değilse döngü başa döner
                player_choice = input(
                    " 'rock', 'paper', 'scissors' or if you want to leave 'exit': ").lower()  # Tekrar girdi isteriz
            ...
            print(
                f"Tour Result: Player {player_tour_win} - {computer_tour_win} Computer\n")  # tur sonucu heaplama ve ekrana yazdırma
        ...
        print(
            f"-----Toplam Oynanan Oyun Sayısı: {played_games} -----")  # Toplam oynanan oyun sayısını ekrana yazdırırız
        print(
            f"**** OYUN SONUCU: Oyuncu {player_wins} - {computer_wins} Bilgisayar ****\n ")  # Genel oyun sonucunu ekrana yazdırırız

        # Yeni bir oyun oynamak isteyip istemediğini oyuncuya sorarız
        print("----------------------------------------------------")
        play_request = input(
            "Başka bir oyun daha oynamak ister misin? (Y/N): ").lower()  # Oyuncuya yeni bir oyun oynamak isteyip istemediğini sorarız
...
# -------------------------------------------------------------------------
def game_loop():  # Oyun döngüsünü başlatan fonksiyon
    """Oyun döngüsünü çalıştırır, her oyun için sayaçları sıfırlar ve genel galibi belirler."""
    global player_wins, computer_wins, played_games, game_round, game
    ...
    while True:  # Sonsuz döngü, oyuncu oyunu bitirene kadar devam eder
        ...
        while player_tour_win < 2 and computer_tour_win < 2:  # Oyuncu veya bilgisayar iki tur kazanana kadar döngü devam eder
            game_round += 1  # Round sayacını bir arttırıyoruz
            print("----------------------------------------------------")
            print(f"##### GAME : {game}, ROUND: {game_round} ######")  # Oyun ve round bilgisini ekrana yazdırıyoruz
            ...
            # Tur sonucunu ekrana yazdırırız
            print(
                f"Tour Result: Player {player_tour_win} - {computer_tour_win} Computer\n")  # Tur sonucu hesaplama ve ekrana yazdırma
        ...
        while True:
            try:
                ...
                if play_request not in ["y", "n"]:
                    play_request = input("Yanlış cevap girdiniz. Lütfen 'Y' veya 'N' giriniz.")
                    if play_request == "Y":
                        continue
                    elif play_request == "N" or play_request not in ["y", "n"]:
                        break
                break  # Eğer cevap 'Y' veya 'N' ise döngüden çıkılır
            except ValueError:
                break  # Hatalı girişi kullanıcıya bildiririz

        if play_request == "n":  # Eğer cevap 'N' ise
            break  # Döngüyü kırarız, oyun sona erer
